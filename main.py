...
PV_THRESH     = 0.65
NMS_IOU       = 0.45
PROCESS_EVERY = 3       # Jetson: skip more frames
DISPLAY_W     = 960
DISPLAY_H     = 720
IMGSZ         = 416     # Jetson: smaller = faster
...
CLASS_COLORS = {
    'priority_vehicle': (0,   0,   255),
    'auto-rickshaw':    (0,   165, 255),
    'bus':              (255, 0,   0  ),
    'car':              (0,   255, 0  ),
...
def draw_overlay(frame, counts, active_lane,
    ...
    sc       = (0, 220, 0) if is_green else (0, 0, 255)
...
def run_detection(model, frame):
    results = model(
        ...
        conf=CONF_THRESH,
        iou=NMS_IOU,
        imgsz=IMGSZ,    # 416 for Jetson speed
        verbose=False
    )[0]
...
def main():
    ...
    while True:
        ...
        # ── Detection — ACTIVE LANE ONLY ──────────────
        # Jetson can't handle all 4 lanes simultaneously
        ...
        if frame_idx % PROCESS_EVERY == 0:
            ...
            # Detect only on active lane
            frame = frames[active_lane]
            counts, boxes_data, pv = run_detection(
                model, frame)

            all_lane_counts[active_lane] = counts
            cong, _ = get_congestion(counts)
            lane_congestion[active_lane] = cong

            if pv:
                priority_detected = True
                controller.set_priority_vehicle(active_lane)
            else:
                controller.reset_priority_vehicle()

            # Annotate active lane frame
            annotated = frames[active_lane].copy()
            annotated = draw_detections(annotated, boxes_data)
            lane_frames[active_lane] = annotated

            # Store raw frames for other lanes
            for lane in LANES:
                if lane != active_lane:
                    lane_frames[lane] = frames[lane].copy()
            ...
            green_dur  = controller.calculate_green_time(
                dict(all_lane_counts[active_lane]))
            ...
            print(f"[{active_lane}] "
                  f"→{green_lane}({time_remaining}s) | "
                  f"{lane_summary} | "
                  f"{last_latency:.0f}ms | "
                  f"{fps:.1f}fps{pv_tag}")

        else:
            # Recalculate timer every frame
            time_remaining = max(
                0, int(_current_dur -
                       (time.time() - _last_switch)))
            last_remaining = time_remaining
        ...
        display = draw_overlay(
            display, active_counts, active_lane,
            last_green, last_duration, last_remaining,
            last_latency, fps, last_priority
        )

        cv2.imshow(
            'Smart Traffic Management System', display)

        key = cv2.waitKey(1) & 0xFF   # delay=1 for Jetson
        if key == ord('q'):
            print("\n👋 Quit")
            break
        elif key == ord('r'):
